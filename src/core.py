...
class TimeSeriesCV:
    ...
    def plot_splits(self, output_path: Path | None = None, plot: bool = False) -> None:
        ...
        if not plot:
            return
        fig, axs = plt.subplots(self.n_splits, 1, figsize=(15, 5 * self.n_splits))
        if self.n_splits == 1:
            axs = [axs]
...
class RollingWindowCV:
    ...
    def _calculate_metrics(y_true: pd.Series, y_pred: np.ndarray) -> dict[str, float]:
        ...
        return {'mse': mean_squared_error(y_true, y_pred), 'mae': mean_absolute_error(y_true, y_pred), 'rmse': np.sqrt(mean_squared_error(y_true, y_pred))}
...
class BlockingTimeSeriesCV:
    ...
    def split(self, data: pd.DataFrame) -> Iterator[tuple[np.ndarray, np.ndarray]]:
        ...
        for i in range(self.n_splits):
            ...
            train_indices = np.concatenate([block for j, block in enumerate(blocks) if j != test_block_idx])
...
class TimeSeriesEvaluation:
    ...
    def add_metric(self, name: str, function: Callable[..., float]):
    ...
    def cross_validate(self, model: Any, cv_splitter: Any, X: pd.DataFrame, y: pd.Series):
        ...
        cv_results: dict[str, list[float]] = {name: [] for name in self.metrics}
...
def check_data_leakage(train_indices: np.ndarray, test_indices: np.ndarray, timestamps: pd.DatetimeIndex) -> bool:
