...
class NestedTimeSeriesCV:
    ...
    def run_nested_cv(
        self, model: Any, param_grid: dict, X: pd.DataFrame, y: pd.Series
    ):
...
class TimeSeriesEvaluation:
    ...
    def cross_validate(
        self, model: Any, cv_splitter: Any, X: pd.DataFrame, y: pd.Series
    ):
...
def check_data_leakage(
    train_indices: np.ndarray, test_indices: np.ndarray, timestamps: pd.DatetimeIndex
) -> bool:
